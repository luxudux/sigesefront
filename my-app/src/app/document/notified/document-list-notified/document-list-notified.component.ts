...
import { DocumentExecutedComponent } from '../document-executed/document-executed.component';
...
@Component({
  selector: 'app-document-list-notified',
  templateUrl: './document-list-notified.component.html',
  styleUrls: ['./document-list-notified.component.css']
})
export class DocumentListNotifiedComponent implements OnInit {
  ...
  loadingList() {
    ...
    // OBSERVABLE
    this.NotificadoService.getItems().subscribe(
      // CORRECT
      (response) => {
        this.cargando = false;
        /* tslint:disable:no-string-literal */
        this.NotificadoService.setItems(response['data']);
  ...
  // MODAL: Executed
  openDialogExecuted(row: any[]): void {

    this.row = row;

    const dialogRef = this.dialog.open(DocumentExecutedComponent, {
      // data: { id: this.id, name: this.name, acronym: this.acronym },
      data: this.row,
      // height: '400px',
      width: '600px',
